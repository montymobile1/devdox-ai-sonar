...
from devdox_ai_sonar.llm import validation
...
def _install_inference(monkeypatch, fn):
    monkeypatch.setattr(provider_inference, "infer_provider", fn)


def _install_verified(monkeypatch, data):
    monkeypatch.setattr(verified_models, "list_providers", lambda: data)
