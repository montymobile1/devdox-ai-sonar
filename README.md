...
graph TB
    subgraph ENTRY ["Entry Point"]
        CLI["cli.py\nClick commands + interactive menus"]
    end

    subgraph SERVICES ["Services"]
        CS["ConfigService\nLoad/save auth + config"]
        RA["RuleAnalyzer\nFetch rules from SonarCloud"]
        IE["IssueExtractor\nCode matching (exact/fuzzy)"]
        RH["RuleHandlerRegistry\nChain of Responsibility"]
    end

    subgraph HANDLERS ["Rule Handlers"]
        H1["AsyncToSyncHandler\npython:S7503"]
        H2["CognitiveComplexityHandler\npython:S3776"]
        H3["DefaultRuleHandler\nAll other rules"]
    end

    subgraph CORE ["Core"]
        SA["SonarCloudAnalyzer\nREST API client"]
        LF["LLMFixer\nPrompt assembly + fix application"]
        FV["FixValidator\nAI code reviewer"]
    end

    subgraph MODELS ["Models (Pydantic)"]
        M1["SonarIssue · AnalysisResult"]
        M2["FixSuggestion · FixResult"]
        M3["ConfigManager · LLMProviderConfig"]
    end
    ...
    style CLI fill:#4a90d9,stroke:#2c6faa,color:#fff
    style CS fill:#7b68ee,stroke:#5b48ce,color:#fff
    style RA fill:#7b68ee,stroke:#5b48ce,color:#fff
    style IE fill:#7b68ee,stroke:#5b48ce,color:#fff
    style RH fill:#7b68ee,stroke:#5b48ce,color:#fff
    style H1 fill:#e67e22,stroke:#c96e1c,color:#fff
    style H2 fill:#e67e22,stroke:#c96e1c,color:#fff
    style H3 fill:#e67e22,stroke:#c96e1c,color:#fff
    style SA fill:#9b59b6,stroke:#7d3c98,color:#fff
    style LF fill:#9b59b6,stroke:#7d3c98,color:#fff
    style FV fill:#9b59b6,stroke:#7d3c98,color:#fff
    style M1 fill:#50c878,stroke:#3da85e,color:#fff
    style M2 fill:#50c878,stroke:#3da85e,color:#fff
    style M3 fill:#50c878,stroke:#3da85e,color:#fff
```
...
```
src/devdox_ai_sonar/
├── cli.py                    # CLI entry point (Click + Questionary + Rich)
├── config.py                 # Settings (Pydantic BaseSettings, paths)
├── sonar_analyzer.py         # SonarCloud REST API client
├── sonar_fetcher.py          # SonarCloud data fetching helpers
├── llm_fixer.py              # LLM fix generation and application
├── fix_validator.py          # AI validation agent
├── models/                   # Pydantic data models
├── services/
│   ├── configuration.py      # Auth config loading/saving
│   ├── extractor.py          # Code extraction and matching
│   ├── rule_analyzer.py      # SonarCloud rule fetching
│   └── rule_handler.py       # Rule handler registry and implementations
├── utils/
│   ├── async_file_io.py      # Async file operations (aiofiles)
│   ├── file_filter.py        # Suffix/prefix file filtering
│   ├── file_indentation.py   # Indentation detection and normalization
│   ├── function_finder.py    # AST-based function boundary detection
│   ├── result.py             # Ok/Err result type
│   ├── ui.py                 # Terminal UI helpers
│   ├── validator.py          # Input validation
│   ├── provider_config.py    # LLM provider configuration
│   ├── sonar_config.py       # SonarCloud configuration
│   ├── constant.py           # String constants
│   └── exceptions.py         # Custom exception classes
├── prompts/python/           # Jinja2 prompt templates
│   ├── system_fix_issues.j2
│   ├── user_prompt.j2
│   ├── validator.j2
│   └── refactoring/          # Specialized prompts for S3776
└── templates/md.j2           # Changelog markdown template
```
...
The project uses [Black](https://github.com/psf/black) and [Ruff](https://github.com/astral-sh/ruff) for formatting, Ruff and [mypy](https://github.com/python/mypy) for linting, and [pytest](https://github.com/pytest-dev/pytest) for testing. These are configured in `pyproject.toml` under `[tool.black]`, `[tool.isort]`, and `[tool.mypy]`.

All quality checks are defined as targets in the `Makefile`:

```bash
make format   # black src && ruff check --fix src && ruff format src
make lint     # ruff check src && mypy --package devdox_ai_sonar
make test     # pytest
```

The publish workflow (`.github/workflows/publish.yml`) runs `make format`, `make lint`, and `make test-fast` as a gate before building and publishing to PyPI. Code that fails any of these checks cannot be released. Run all three before submitting a pull request:
...
|---|---|---|
| `build.yml` | Every push and PR | Run tests with coverage, upload coverage report, SonarQube scan |
| `publish.yml` | GitHub release or manual dispatch | `make format` + `make lint` + `make test-fast`, build package, publish to PyPI |
