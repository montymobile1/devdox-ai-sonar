...
def validate_model_string(
    ...
    *,
    ...
    provider = adapters.infer_provider(model=model, api_base=base_url)
...
def probe_api_key(
    ...
    *,
    ...
    outcome = adapters.probe(model=model, api_key=api_key, api_base=base_url)
    if outcome.ok:
        return Ok(None)
...
def _is_openhands_verified(model: str) -> bool:
    ...
    :func:`adapters.infer_provider` for this family -- we look in the
    verified catalog directly instead.
    """
    ...
    if not model_id:
        return False
    verified = adapters.list_verified_models().get(_OPENHANDS_FAMILY, [])
